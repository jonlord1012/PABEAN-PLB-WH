...
class Mrpt_doc_fcmr extends Model
{
    ...
    public static function load_edit_dokumen($param)
    {
        ...
        $getdata = DB::table('fcmr_header')->where('dokumen_no', '=', $vdata['dokumen_no'])->first();
        return json_encode([
            'success' => 'true',
    ...
    {
        $vdata = json_decode($param['vdata'], true);
        $getdata = DB::table('fcmr_detail')->where(['dokumen_no' => $vdata['dokumen_no'], 'period' => $vdata['period']])->get();
        return json_encode([
