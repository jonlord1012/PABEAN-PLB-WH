...
class Mmst_group_item extends Model
{
  ...
  {
    try {
      $query = DB::table('vw_mst_part_group');
  ...
  public static function proses_data_insert($param)
  {
    ...
    $check = DB::table('vw_mst_part_group')
    ...
    if ($check > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode atau Nama Group sudah ada!',
        'vdata' => null
      ]);
    }
    ...
    // Data untuk insert
    $insert_data = [
      'defmodule' => 'GROUP PART',
      'defcode' => $vdata['defcode'],
      'defname' => $vdata['defname'],
    ];

    $insert_id = DB::table('vw_mst_part_group')->insertGetId($insert_data);

    // Ambil ID terakhir
    if ($insert_id) {
      // Ambil data yang baru disimpan
      $getdata = DB::table('vw_mst_part_group')
  ...
  public static function proses_data_update($param)
  {
    ...
    if (empty($vdata['defcode']) || empty($vdata['defname'])) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan, Group dan nama Group harus diisi',
        'vdata' => null
      ]);
    }

    // Cek duplikasi kode Base di record lain
    $check_code = DB::table('vw_mst_part_group')
      ->where('defcode', $vdata['defcode'])
    ...
    if ($check_code > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode Group sudah digunakan!',
        'vdata' => null
      ]);
    }

    // Cek duplikasi nama Base di record lain
    $check_name = DB::table('vw_mst_part_group')
    ...
    if ($check_name > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Nama Group sudah digunakan!',
        'vdata' => null
      ]);
    }
    ...
    $updated = DB::table('vw_mst_part_group')
  ...
  public static function delete_data($param)
  {
    ...
    $deleted = DB::table('vw_mst_part_group')
      ->where('defid', $vdata['defid'])
      ->delete();
  ...
  public static function download_data($param)
  {
      $query = DB::table('vw_mst_part_group')
          ->select(
              'defcode as Kode Group Part',
              'defname as Nama Group Part',
          );
      ...
      $date = new DateTime();
      $vfilename = "result_group_part_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
      $outputFilePath = base_path("../z_download/" . $vfilename);

      $writer->openToFile($outputFilePath);

      $firstSheet = $writer->getCurrentSheet();
      $firstSheet->setName('Data Group Part');
      $header = ['Kode Group Part', 'Nama Group Part' ];
