...
class Mrpt_doc_asr extends Model
{
    ...
    public static function load_edit_dokumen($param)
    {
        ...
        $getdata = DB::table('asr_header')->where('dokumen_no', '=', $vdata['dokumen_no'])->first();
        return json_encode([
            'success' => 'true',
    ...
    {
        $vdata = json_decode($param['vdata'], true);
        $getdata = DB::table('asr_detail')->where(['dokumen_no' => $vdata['dokumen_no'], 'period' => $vdata['period']])->get();
        return json_encode([
