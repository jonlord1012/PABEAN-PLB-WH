...
class Masset_group extends Model
{
    ...
    {
        $query = DB::table('cpmatrix')
            ->select('defcode as Group Code', 'defname as Group Name')
            ->where('defmodule', '=', 'MGROUP');
        ...
        $date = new DateTime();
        $vfilename = "data_asset_group_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        $existingGroup = DB::table('cpmatrix')
            ->where('defmodule', $field['defmodule'])
            ->where('defcode', $vdata['defcode']);

        if (!empty($vdata['defid'])) {
            $existingGroup->where('defid', '!=', $vdata['defid']);
        }
        $existingRecord = $existingGroup->first();
        if ($existingGroup->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Group Code " . $existingRecord->defcode . " sudah ada"
            ]);
        }

        if (!empty($vdata['defid'])) {
            $updatedGroup = DB::table('cpmatrix')->where('defid', $vdata['defid'])->update(array_filter($field_update));
            ...
            $createdGroup = DB::table('cpmatrix')->insert(array_filter($field));
