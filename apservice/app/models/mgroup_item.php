...
class Mgroup_item extends Model
{
    ...
    public static function read_data($param)
    {
        ...
        $query = DB::table('vw_item_group as c')
    ...
    public static function download_data($param)
    {
        ...
        $query = DB::table('vw_item_group as c')
            ->select(
                'c.defcode as Group Item Code',
                'c.defname as Group Item Name',
                'd.deptname as Dept'
            )
            ->join('vw_department as d', 'd.deptcode', '=', 'c.defdescription');
        if ($vprofile['usergroup'] !== 'GROUP_ADMIN') {
        ...
        $date = new DateTime();
        $vfilename = "data_group_item_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        if ($existingCostcenter->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Group Item Name " . $existingRecord->defname . " sudah ada"
            ]);
        }
