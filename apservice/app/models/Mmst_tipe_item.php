...
class Mmst_tipe_item extends Model
{
  ...
  {
    try {
      $query = DB::table('vw_mst_part_type');
  ...
  public static function proses_data_insert($param)
  {
    ...
    $check = DB::table('vw_mst_part_type')
    ...
    if ($check > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode atau Nama Tipe sudah ada!',
        'vdata' => null
      ]);
    }
    ...
    $insert_id = DB::table('vw_mst_part_type')->insertGetId($insert_data);

    // Ambil ID terakhir
    if ($insert_id) {
      // Ambil data yang baru disimpan
      $getdata = DB::table('vw_mst_part_type')
  ...
  public static function proses_data_update($param)
  {
    ...
    if (empty($vdata['defcode']) || empty($vdata['defname'])) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan, tipe dan nama tipe harus diisi',
        'vdata' => null
      ]);
    }

    // Cek duplikasi kode tipe di record lain
    $check_code = DB::table('vw_mst_part_type')
      ->where('defcode', $vdata['defcode'])
    ...
    if ($check_code > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode Tipe sudah digunakan!',
        'vdata' => null
      ]);
    }

    // Cek duplikasi nama tipe di record lain
    $check_name = DB::table('vw_mst_part_type')
    ...
    if ($check_name > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Nama Tipe sudah digunakan!',
        'vdata' => null
      ]);
    }
    ...
    $updated = DB::table('vw_mst_part_type')
  ...
  public static function delete_data($param)
  {
    ...
    $deleted = DB::table('vw_mst_part_type')
      ->where('defid', $vdata['defid'])
      ->delete();
  ...
  public static function download_data($param)
  {
      $query = DB::table('vw_mst_part_type')
          ->select(
              'defcode as Kode Tipe Part',
              'defname as Nama Tipe Part',
          );
      ...
      $date = new DateTime();
      $vfilename = "result_type_part_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
      $outputFilePath = base_path("../z_download/" . $vfilename);

      $writer->openToFile($outputFilePath);

      $firstSheet = $writer->getCurrentSheet();
      $firstSheet->setName('Data Tipe Part');
      $header = ['Kode Tipe Part', 'Nama Tipe Part' ];
