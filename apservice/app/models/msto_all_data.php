...
class Msto_all_data extends Model
{
    ...
    {
        $activePeriod = DB::table('speriod')
            ->where('status', 'OPEN')
            ->first();

        $totalAssets = DB::table('stodata')->where("period", $activePeriod->period)->count();

        $scannedAssets = DB::table('stodata')->where("period", $activePeriod->period)->whereNotNull('scan_date')->count();

        $query = DB::table('stodata')->select('*')->where("period", $activePeriod->period);
        ...
        return json_encode([
            ...
            'AssetsNotScanned' => $totalAssets - $scannedAssets
        ]);
    }
