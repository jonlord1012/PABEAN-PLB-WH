    ...
    public static function read_data($param)
    {
        $query = DB::table('assetdata')
            ->select(
                ...
                'derived_table.b_period as period',
                'assetdata.assetno',
                'assetdata.assetkey',
            ...
            ->join(
                DB::raw("(WITH derived_table AS (
                            SELECT A.assetno, A.period, B.period AS b_period
                            FROM stodata A
                            LEFT JOIN speriod B
                            ...
                            AND (A.period <> '' AND A.period IS NOT NULL)
                            GROUP BY A.assetno, A.period, B.period
                        ) SELECT * FROM derived_table) AS derived_table"),
                'assetdata.assetno',
                '=',
                'derived_table.assetno'
            )
            ->whereColumn('assetdata.assetcondition', '<>', 'derived_table.b_period');
