...
class Mmst_category_rack extends Model
{
  ...
  {
    try {
      $query = DB::table('vw_mst_rack_category');

      // Filter
      if (array_key_exists('filter', $param)) {
        $keyval = json_decode($param['filter'], true);
        foreach ($keyval as $key => $val) {
          $colname = ['create_date', 'sysupdatedate'];
  ...
  public static function read_data_category($param)
  {
    try {
      $query = DB::table('vw_mst_rack_category');
  ...
  public static function proses_data_insert($param)
  {
    ...
    $check = DB::table('vw_mst_rack_category')
    ...
    // Data untuk insert
    $insert_data = [
      'defmodule' => 'CATEGORY_RACK',
      'defcode' => $vdata['defcode'],
      'defname' => $vdata['defname'],
    ];

    $insert_id = DB::table('vw_mst_rack_category')->insertGetId($insert_data);

    // Ambil ID terakhir
    if ($insert_id) {
      // Ambil data yang baru disimpan
      $getdata = DB::table('vw_mst_rack_category')
  ...
  public static function proses_data_update($param)
  {
    ...
    $check_code = DB::table('vw_mst_rack_category')
      ->where('defcode', $vdata['defcode'])
    ...
    $check_name = DB::table('vw_mst_rack_category')
    ...
    $updated = DB::table('vw_mst_rack_category')
  ...
  public static function delete_data($param)
  {
    ...
    $deleted = DB::table('vw_mst_rack_category')
      ->where('defid', $vdata['defid'])
      ->delete();
  ...
  public static function read_data_bycategory($param)
  {
    try {
      $vdata = json_decode($param['vdata'], true);
      $query = DB::table('mst_rack')
        ->where('rack_category', $vdata['defcode']);
  ...
  public static function download_data($param)
  {
      $query = DB::table('vw_mst_rack_category')
          ->select(
              'defcode as Kode Kategori Rack',
              'defname as Nama Kategori Rack',
          );
      ...
      $date = new DateTime();
      $vfilename = "result_rack_part_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
      $outputFilePath = base_path("../z_download/" . $vfilename);

      $writer->openToFile($outputFilePath);

      $firstSheet = $writer->getCurrentSheet();
      $firstSheet->setName('Data Category Rack');
      $header = ['Kode Kategori Rack', 'Nama Kategori Rack' ];
