...
class Mmst_base_item extends Model
{
  ...
  {
    try {
      $query = DB::table('vw_mst_part_base');
  ...
  public static function proses_data_insert($param)
  {
    ...
    $check = DB::table('vw_mst_part_base')
    ...
    if ($check > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode atau Nama Base sudah ada!',
        'vdata' => null
      ]);
    }
    ...
    $insert_id = DB::table('vw_mst_part_base')->insertGetId($insert_data);

    // Ambil ID terakhir
    if ($insert_id) {
      // Ambil data yang baru disimpan
      $getdata = DB::table('vw_mst_part_base')
  ...
  public static function proses_data_update($param)
  {
    ...
    if (empty($vdata['defcode']) || empty($vdata['defname'])) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan, Base dan nama Base harus diisi',
        'vdata' => null
      ]);
    }

    // Cek duplikasi kode Base di record lain
    $check_code = DB::table('vw_mst_part_base')
      ->where('defcode', $vdata['defcode'])
    ...
    if ($check_code > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode Base sudah digunakan!',
        'vdata' => null
      ]);
    }

    // Cek duplikasi nama Base di record lain
    $check_name = DB::table('vw_mst_part_base')
    ...
    if ($check_name > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Nama Base sudah digunakan!',
        'vdata' => null
      ]);
    }
    ...
    $updated = DB::table('vw_mst_part_base')
  ...
  public static function delete_data($param)
  {
    ...
    $deleted = DB::table('vw_mst_part_base')
      ->where('defid', $vdata['defid'])
      ->delete();
  ...
  public static function download_data($param)
  {
      $query = DB::table('vw_mst_part_base')
          ->select(
              'defcode as Kode Base Part',
              'defname as Nama Base Part',
          );
      ...
      $date = new DateTime();
      $vfilename = "result_base_part_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
      $outputFilePath = base_path("../z_download/" . $vfilename);

      $writer->openToFile($outputFilePath);

      $firstSheet = $writer->getCurrentSheet();
      $firstSheet->setName('Data Baset Part');
      $header = ['Kode Base Part', 'Nama Base Part' ];
