...
class Mmsublocation extends Model
{
    ...
    {
        $query = DB::table('cpmatrix')
            ->select('defname as Sub Location name', 'defcode as Sub Location Code')
            ->where('defmodule', '=', 'MSUBLOCATION');
        ...
        $date = new DateTime();
        $vfilename = "data_asset_sub_location_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        $existingSubLocation = DB::table('cpmatrix')->where('defmodule', $field['defmodule'])
            ->where('defname', $vdata['defname']);

        if (!empty($vdata['defid'])) {
            $existingSubLocation->where('defid', '!=', $vdata['defid']);
        }

        $existingRecord = $existingSubLocation->first();
        if ($existingSubLocation->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Sub Location name " . $existingRecord->defname . " sudah ada"
            ]);
        }

        if (!empty($vdata['defid'])) {
            $updatedSubLocation = DB::table('cpmatrix')->where('defid', $vdata['defid'])->update(array_filter($field));
            ...
            $createdSubLocation = DB::table('cpmatrix')->insert(array_filter($field));
