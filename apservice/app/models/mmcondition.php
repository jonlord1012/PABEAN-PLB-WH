...
class Mmcondition extends Model
{
    ...
    {
        $query = DB::table('cpmatrix')
            ->select('defcode as Condition Code', 'defname as Condition Name', )
            ->where('defmodule', '=', 'MCONDITION');
        ...
        $date = new DateTime();
        $vfilename = "data_asset_condition_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        $existingCondition = DB::table('cpmatrix')->where('defmodule', $field['defmodule'])
            ->where('defname', $vdata['defname']);

        if (!empty($vdata['defid'])) {
            $existingCondition->where('defid', '!=', $vdata['defid']);
        }

        $existingRecord = $existingCondition->first();
        if ($existingCondition->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Condition name " . $existingRecord->defname . " sudah ada"
            ]);
        }

        if (!empty($vdata['defid'])) {
            $updatedCondition = DB::table('cpmatrix')->where('defid', $vdata['defid'])->update(array_filter($field));
            ...
            $createdCondition = DB::table('cpmatrix')->insert(array_filter($field));
