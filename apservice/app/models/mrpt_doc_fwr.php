...
class Mrpt_doc_fwr extends Model
{
    ...
    public static function load_edit_dokumen($param)
    {
        ...
        $getdata = DB::table('fwr_header')->where('dokumen_no', '=', $vdata['dokumen_no'])->first();
        return json_encode([
            'success' => 'true',
    ...
    {
        $vdata = json_decode($param['vdata'], true);
        $getdata = DB::table('fwr_detail')->where(['dokumen_no' => $vdata['dokumen_no'], 'period' => $vdata['period']])->get();
        return json_encode([
