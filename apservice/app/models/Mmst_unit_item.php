...
class Mmst_unit_item extends Model
{
  ...
  {
    try {
      $query = DB::table('vw_mst_part_uom');
  ...
  public static function proses_data_insert($param)
  {
    ...
    $check = DB::table('vw_mst_part_uom')
    ...
    if ($check > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode atau Nama Unit sudah ada!',
        'vdata' => null
      ]);
    }
    ...
    $insert_id = DB::table('vw_mst_part_uom')->insertGetId($insert_data);

    // Ambil ID terakhir
    if ($insert_id) {
      // Ambil data yang baru disimpan
      $getdata = DB::table('vw_mst_part_uom')
  ...
  public static function proses_data_update($param)
  {
    ...
    if (empty($vdata['defcode']) || empty($vdata['defname'])) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan, Unit dan nama Unit harus diisi',
        'vdata' => null
      ]);
    }

    // Cek duplikasi kode Unit di record lain
    $check_code = DB::table('vw_mst_part_uom')
      ->where('defcode', $vdata['defcode'])
    ...
    if ($check_code > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Kode Unit sudah digunakan!',
        'vdata' => null
      ]);
    }

    // Cek duplikasi nama Unit di record lain
    $check_name = DB::table('vw_mst_part_uom')
    ...
    if ($check_name > 0) {
      return json_encode([
        'success' => 'false',
        'message' => 'Data gagal disimpan. Nama Unit sudah digunakan!',
        'vdata' => null
      ]);
    }
    ...
    $updated = DB::table('vw_mst_part_uom')
  ...
  public static function delete_data($param)
  {
    ...
    $deleted = DB::table('vw_mst_part_uom')
      ->where('defid', $vdata['defid'])
      ->delete();
  ...
  public static function download_data($param)
  {
      $query = DB::table('vw_mst_part_uom')
          ->select(
              'defcode as Kode UOM Part',
              'defname as Nama UOM Part',
          );
      ...
      $date = new DateTime();
      $vfilename = "result_uom_part_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
      $outputFilePath = base_path("../z_download/" . $vfilename);

      $writer->openToFile($outputFilePath);

      $firstSheet = $writer->getCurrentSheet();
      $firstSheet->setName('Data Unit Part');
      $header = ['Kode UOM Part', 'Nama UOM Part' ];
