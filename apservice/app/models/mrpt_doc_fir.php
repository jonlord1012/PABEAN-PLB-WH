...
class Mrpt_doc_fir extends Model
{
    ...
    public static function load_edit_dokumen($param)
    {
        ...
        $getdata = DB::table('fir_header')->where('dokumen_no', '=', $vdata['dokumen_no'])->first();
        return json_encode([
            'success' => 'true',
    ...
    {
        $vdata = json_decode($param['vdata'], true);
        $getdata = DB::table('fir_detail')->where('tbid', '=', $vdata['tbid'])->first();
        return json_encode([
            'success' => 'true',
            'message' => 'Data ditampilkan',
            'vdata' => json_encode((array) $getdata)
    ...
    {
        $vdata = json_decode($param['vdata'], true);
        $getdata = DB::table('fir_detail')->where(['dokumen_no' => $vdata['dokumen_no'], 'period' => $vdata['period']])->get();
        return json_encode([
