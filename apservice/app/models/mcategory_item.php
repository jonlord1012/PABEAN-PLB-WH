...
class Mcategory_item extends Model
{
    ...
    public static function read_data($param)
    {
        ...
        $query = DB::table('vw_item_category as c')
    ...
    public static function download_data($param)
    {
        ...
        $query = DB::table('vw_item_category as c')
            ->select(
                'c.defcode as Category Item Code',
                'c.defname as Category Item Name',
                'd.deptname as Dept'
            )
            ->join('vw_department as d', 'd.deptcode', '=', 'c.defdescription');
        if ($vprofile['usergroup'] !== 'GROUP_ADMIN') {
        ...
        $date = new DateTime();
        $vfilename = "data_category_item_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        if ($existingCostcenter->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Category Item Name " . $existingRecord->defname . " sudah ada"
            ]);
        }
