...
class Masset_category extends Model
{
    ...
    {
        $query = DB::table('cpmatrix')
            ->select('defcode as Category Code', 'defname as Category Name')
            ->where('defmodule', '=', 'MCATEGORY');
        ...
        $date = new DateTime();
        $vfilename = "data_asset_category_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        $existingCategory = DB::table('cpmatrix')
            ->where('defmodule', $field['defmodule'])
            ->where('defcode', $vdata['defcode']);

        if (!empty($vdata['defid'])) {
            $existingCategory->where('defid', '!=', $vdata['defid']);
        }

        $existingRecord = $existingCategory->first();
        if ($existingCategory->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Category Code " . $existingRecord->defcode . " sudah ada"
            ]);
        }

        if (!empty($vdata['defid'])) {
            $updatedCategory = DB::table('cpmatrix')->where('defid', $vdata['defid'])->update(array_filter($field_update));
            ...
            $createdCategory = DB::table('cpmatrix')->insert(array_filter($field));
    ...
    public static function delete_data($param)
    {
        $vdata = $param['defid'];
        try {
            $deleted = DB::table('cpmatrix')->where('defid', $vdata)->delete();
