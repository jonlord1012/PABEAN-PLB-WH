...
class Msubcategory_item extends Model
{
    ...
    {
        $vprofile = json_decode($param['vprofile'], true);

        $query = DB::table('vw_item_subcategory as c')
            ->select('c.defid', 'c.defname', 'c.defcode', 'c.defdescription', 'd.deptname')
            ->join('vw_department as d', 'd.deptcode', '=', 'c.defdescription');

        if ($vprofile['usergroup'] !== 'GROUP_ADMIN') {
            $query->whereIn('c.defdescription', $vprofile['userdept']);
        }
    ...
    public static function download_data($param)
    {
        ...
        $query = DB::table('vw_item_subcategory as c')
            ->select(
                'c.defcode as Sub Category Item Code',
                'c.defname as Sub Category Item Name',
                'd.deptname as Dept'
            )
            ->join('vw_department as d', 'd.deptcode', '=', 'c.defdescription');
        if ($vprofile['usergroup'] !== 'GROUP_ADMIN') {
        ...
        $date = new DateTime();
        $vfilename = "data_subcategory_item_download_" . $date->format('Y_m_d_H_i_s') . ".xlsx";
    ...
    public static function save_data($param)
    {
        ...
        $field = array(
            ...
            'defdescription' => $vdata['defdescription'],
        ...
        if ($existingCostcenter->exists()) {
            return json_encode([
                'success' => false,
                'message' => "Sub Category Item Name " . $existingRecord->defname . " sudah ada"
            ]);
        }
