...
class Masset_data_new extends Model
{
    ...
    public static function read_data($param)
    {
        ...
        $query = DB::table('tr_asset as A')
            ->select('A.*', 'A.asset_amount as asset_cost', 'B.asset_acquisition_date')
            ->leftJoin('tr_asset_finance as B', 'A.asset_no', '=', 'B.asset_no')
    ...
    public static function read_data_sub_asset($param)
    {
        ...
        $doktype = "baab";
        if (substr($vdata['asset_no'], 0, 1) == '9') {
            $doktype = "flr";
        }
        $query = DB::table($doktype . '_detailsub')
            ->select('*')
