...
Ext.define("NJC.rpt_out_material.FRMrpt_out_material", {
  extend: "Ext.window.Window",
  alias: "widget.FRMrpt_out_material",
  reference: "FRMrpt_out_material",
  title: "Report Out Material",
  modal: true,
  closeAction: "destroy",
  centered: true,
      ...
      items: [
        {
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "rpt_out_material/rpt_out_materials",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMrpt_out_material]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.rpt_out_material.FRMrpt_out_material_list_aju",
        {}
      );
      var GRIDrpt_out_material = FRMmain.query(
        "grid[pid=GRIDFRMrpt_out_material]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDrpt_out_material.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDrpt_out_material.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDrpt_out_material = MODULEmain.query(
        "grid[pid=GRIDrpt_out_material]"
      )[0];
      var GRIDFRMrpt_out_material = PAGEthis.query(
        "grid[pid=GRIDFRMrpt_out_material]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMrpt_out_material.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "rpt_out_material/rpt_out_material",
              params,
              "POST",
