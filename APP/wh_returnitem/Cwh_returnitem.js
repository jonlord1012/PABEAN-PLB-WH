Ext.define("NJC.wh_returnitem.Cwh_returnitem", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cwh_returnitem",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cwh_returnitem");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDwh_returnitem]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("wh_returnitem");
      var popup = Ext.create("NJC.wh_returnitem.FRMwh_returnitem", {});
  ...
  GRIDwh_returnitem_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("wh_returnitem");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("wh_returnitem");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.wh_returnitem.FRMwh_returnitem", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMwh_returnitem = popup.query("grid[pid=GRIDFRMwh_returnitem]")[0];
      GRIDFRMwh_returnitem.getStore().on(
        "beforeload",
      ...
      GRIDFRMwh_returnitem.getStore().load();
