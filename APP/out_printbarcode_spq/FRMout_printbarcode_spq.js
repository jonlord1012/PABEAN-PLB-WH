...
Ext.define("NJC.out_printbarcode_spq.FRMout_printbarcode_spq", {
  extend: "Ext.window.Window",
  alias: "widget.FRMout_printbarcode_spq",
  reference: "FRMout_printbarcode_spq",
  title: "Input Request Material",
  modal: true,
  closeAction: "destroy",
  centered: true,
  ...
  items: [
    ...
    { xtype: "tbspacer", height: 10 },
    {
      ...
      items: [
        {
          xtype: "grid",
          pid: "GRIDFRMout_printbarcode_spq",
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "out_printbarcode_spq/out_printbarcode_spqs",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMout_printbarcode_spq]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.out_printbarcode_spq.FRMout_printbarcode_spq_list_aju",
        {}
      );
      var GRIDout_printbarcode_spq = FRMmain.query(
        "grid[pid=GRIDFRMout_printbarcode_spq]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDout_printbarcode_spq.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDout_printbarcode_spq.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDout_printbarcode_spq = MODULEmain.query(
        "grid[pid=GRIDout_printbarcode_spq]"
      )[0];
      var GRIDFRMout_printbarcode_spq = PAGEthis.query(
        "grid[pid=GRIDFRMout_printbarcode_spq]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMout_printbarcode_spq.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "out_printbarcode_spq/out_printbarcode_spq",
              params,
              "POST",
