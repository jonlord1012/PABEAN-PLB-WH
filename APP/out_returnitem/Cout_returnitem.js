Ext.define("NJC.out_returnitem.Cout_returnitem", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cout_returnitem",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cout_returnitem");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDout_returnitem]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("out_returnitem");
      var popup = Ext.create("NJC.out_returnitem.FRMout_returnitem", {});
  ...
  GRIDout_returnitem_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("out_returnitem");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("out_returnitem");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.out_returnitem.FRMout_returnitem", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMout_returnitem = popup.query("grid[pid=GRIDFRMout_returnitem]")[0];
      GRIDFRMout_returnitem.getStore().on(
        "beforeload",
      ...
      GRIDFRMout_returnitem.getStore().load();
