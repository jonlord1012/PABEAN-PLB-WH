Ext.define("NJC.rpt_inc_material.Crpt_inc_material", {
  extend: "Ext.app.ViewController",
  alias: "controller.Crpt_inc_material",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Crpt_inc_material");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDrpt_inc_material]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("rpt_inc_material");
      var popup = Ext.create("NJC.rpt_inc_material.FRMrpt_inc_material", {});
  ...
  GRIDrpt_inc_material_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("rpt_inc_material");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("rpt_inc_material");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.rpt_inc_material.FRMrpt_inc_material", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMrpt_inc_material = popup.query("grid[pid=GRIDFRMrpt_inc_material]")[0];
      GRIDFRMrpt_inc_material.getStore().on(
        "beforeload",
      ...
      GRIDFRMrpt_inc_material.getStore().load();
