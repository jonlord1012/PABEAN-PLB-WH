...
Ext.define("NJC.tr_adjustment_stock.FRMtr_adjustment_stock", {
  extend: "Ext.window.Window",
  alias: "widget.FRMtr_adjustment_stock",
  reference: "FRMtr_adjustment_stock",
  title: "Input Adjusment Rack",
  modal: true,
  closeAction: "destroy",
  centered: true,
  ...
  items: [
    ...
    { xtype: "tbspacer", height: 10 },
    {
      ...
      items: [
        {
          xtype: "grid",
          pid: "GRIDFRMtr_adjustment_stock",
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "tr_adjustment_stock/tr_adjustment_stocks",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMtr_adjustment_stock]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.tr_adjustment_stock.FRMtr_adjustment_stock_list_aju",
        {}
      );
      var GRIDtr_adjustment_stock = FRMmain.query(
        "grid[pid=GRIDFRMtr_adjustment_stock]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDtr_adjustment_stock.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDtr_adjustment_stock.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDtr_adjustment_stock = MODULEmain.query(
        "grid[pid=GRIDtr_adjustment_stock]"
      )[0];
      var GRIDFRMtr_adjustment_stock = PAGEthis.query(
        "grid[pid=GRIDFRMtr_adjustment_stock]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMtr_adjustment_stock.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "tr_adjustment_stock/tr_adjustment_stock",
              params,
              "POST",
