Ext.define("NJC.tr_floor_rackstock.Ctr_floor_rackstock", {
  extend: "Ext.app.ViewController",
  alias: "controller.Ctr_floor_rackstock",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Ctr_floor_rackstock");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDtr_floor_rackstock]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("tr_floor_rackstock");
      var popup = Ext.create("NJC.tr_floor_rackstock.FRMtr_floor_rackstock", {});
  ...
  GRIDtr_floor_rackstock_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("tr_floor_rackstock");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("tr_floor_rackstock");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.tr_floor_rackstock.FRMtr_floor_rackstock", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMtr_floor_rackstock = popup.query("grid[pid=GRIDFRMtr_floor_rackstock]")[0];
      GRIDFRMtr_floor_rackstock.getStore().on(
        "beforeload",
      ...
      GRIDFRMtr_floor_rackstock.getStore().load();
