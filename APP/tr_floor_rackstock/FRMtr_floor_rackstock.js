...
Ext.define("NJC.tr_floor_rackstock.FRMtr_floor_rackstock", {
  extend: "Ext.window.Window",
  alias: "widget.FRMtr_floor_rackstock",
  reference: "FRMtr_floor_rackstock",
  title: "Input Floor Rack",
  modal: true,
  closeAction: "destroy",
  centered: true,
      ...
      items: [
        {
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "tr_floor_rackstock/tr_floor_rackstocks",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMtr_floor_rackstock]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.tr_floor_rackstock.FRMtr_floor_rackstock_list_aju",
        {}
      );
      var GRIDtr_floor_rackstock = FRMmain.query(
        "grid[pid=GRIDFRMtr_floor_rackstock]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDtr_floor_rackstock.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDtr_floor_rackstock.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDtr_floor_rackstock = MODULEmain.query(
        "grid[pid=GRIDtr_floor_rackstock]"
      )[0];
      var GRIDFRMtr_floor_rackstock = PAGEthis.query(
        "grid[pid=GRIDFRMtr_floor_rackstock]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMtr_floor_rackstock.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "tr_floor_rackstock/tr_floor_rackstock",
              params,
              "POST",
