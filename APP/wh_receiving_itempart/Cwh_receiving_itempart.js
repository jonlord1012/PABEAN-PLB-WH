Ext.define("NJC.wh_receiving_itempart.Cwh_receiving_itempart", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cwh_receiving_itempart",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cwh_receiving_itempart");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDwh_receiving_itempart]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("wh_receiving_itempart");
      var popup = Ext.create("NJC.wh_receiving_itempart.FRMwh_receiving_itempart", {});
  ...
  GRIDwh_receiving_itempart_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("wh_receiving_itempart");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("wh_receiving_itempart");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.wh_receiving_itempart.FRMwh_receiving_itempart", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMwh_receiving_itempart = popup.query("grid[pid=GRIDFRMwh_receiving_itempart]")[0];
      GRIDFRMwh_receiving_itempart.getStore().on(
        "beforeload",
      ...
      GRIDFRMwh_receiving_itempart.getStore().load();
      ...
      FRM.getForm().setValues({
        ...
        supplier_name: rec.data.supplier_name,
