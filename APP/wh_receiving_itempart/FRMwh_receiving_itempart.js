...
Ext.define("NJC.wh_receiving_itempart.FRMwh_receiving_itempart", {
  extend: "Ext.window.Window",
  alias: "widget.FRMwh_receiving_itempart",
  reference: "FRMwh_receiving_itempart",
  ...
  items: [
                  ...
                  items: [
                    ...
                    },
                    {
                      ...
                      renderer: function (value) {
                        var text = value === null ? null : moment(value).format("YYYY-MM-DD");
                        return text;
                      },
                    },
                  ],
    ...
    { xtype: "tbspacer", height: 10 },
    {
      ...
      items: [
        {
          xtype: "grid",
          pid: "GRIDFRMwh_receiving_itempart",
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "wh_receiving_itempart/wh_receiving_itemparts",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMwh_receiving_itempart]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.wh_receiving_itempart.FRMwh_receiving_itempart_list_aju",
        {}
      );
      var GRIDwh_receiving_itempart = FRMmain.query(
        "grid[pid=GRIDFRMwh_receiving_itempart]"
      )[0];
        ...
        function (xgrid, rec) {
          try {
            ...
            FRM.getForm().setValues({
              ...
              supplier_name: rec.data.nama,
              bc_type: rec.data.kode_dokumen_pabean
            });
            popup.close();

            GRIDwh_receiving_itempart.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDwh_receiving_itempart.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDwh_receiving_itempart = MODULEmain.query(
        "grid[pid=GRIDwh_receiving_itempart]"
      )[0];
      var GRIDFRMwh_receiving_itempart = PAGEthis.query(
        "grid[pid=GRIDFRMwh_receiving_itempart]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMwh_receiving_itempart.getStore().each(function (record) {
        nvdetail.push(record.getData());
      });
      var isValid = true;
      GRIDFRMwh_receiving_itempart.getStore().each(function (record, index) {
        let input_qty = record.get('input_qty'); // Ambil nilai 'input_qty' dari record
        let sisa_qty = record.get('sisa_qty'); // Ambil nilai 'input_qty' dari record
        
        if (!input_qty || input_qty == 0) {
          console.log(input_qty); // Debugging: Lihat nilai input_qty di konsol
          COMP.TipToast.msgbox("Error", `Input Quantity tidak boleh 0 pada data ke-${index + 1}`, {
            cls: "danger",
            delay: 3000,
          });
          isValid = false;
        ...
        if (input_qty != sisa_qty) {
          COMP.TipToast.msgbox("Error", "Input Qty harus sesuai dengan sisa Qty, agar data masuk sesuai dengan dokumen Pabean", { cls: "danger", delay: 2000 });
          isValid = false;
          return false;
        }
      });
      ...
      dtval.receipt_date = dtval.receipt_date ? moment(dtval.receipt_date).format("YYYY-MM-DD") : null;
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "wh_receiving_itempart/wh_receiving_itempart",
              params,
              "POST",
