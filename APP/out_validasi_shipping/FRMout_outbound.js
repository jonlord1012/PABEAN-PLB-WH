...
Ext.define("NJC.out_outbound.FRMout_outbound", {
  extend: "Ext.window.Window",
  alias: "widget.FRMout_outbound",
  reference: "FRMout_outbound",
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.out_outbound.FRMout_outbound_list_aju",
        {}
      );
      var GRIDout_outbound = FRMmain.query(
        "grid[pid=GRIDFRMout_outbound]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDout_outbound.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDout_outbound = MODULEmain.query(
        "grid[pid=GRIDout_outbound]"
      )[0];
      var GRIDFRMout_outbound = PAGEthis.query(
        "grid[pid=GRIDFRMout_outbound]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMout_outbound.getStore().each(function (record) {
