Ext.define("NJC.mst_group_item.Cmst_group_item", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmst_group_item",
  init: function (view) {
    this.control({
      //
    });
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDmst_group_item]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btinput_mst_group_item_click: function (cmp) {
    try {
      var MODULEmain = cmp.up("mst_group_item");
      var popup = Ext.create("NJC.mst_group_item.FRMmst_group_item", {});
  ...
  GRIDmst_group_item_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("mst_group_item");
