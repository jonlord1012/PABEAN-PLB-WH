...
Ext.define("NJC.rpt_material.FRMrpt_material", {
  extend: "Ext.window.Window",
  alias: "widget.FRMrpt_material",
  reference: "FRMrpt_material",
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.rpt_material.FRMrpt_material_list_aju",
        {}
      );
      var GRIDrpt_material = FRMmain.query(
        "grid[pid=GRIDFRMrpt_material]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDrpt_material.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDrpt_material = MODULEmain.query(
        "grid[pid=GRIDrpt_material]"
      )[0];
      var GRIDFRMrpt_material = PAGEthis.query(
        "grid[pid=GRIDFRMrpt_material]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMrpt_material.getStore().each(function (record) {
