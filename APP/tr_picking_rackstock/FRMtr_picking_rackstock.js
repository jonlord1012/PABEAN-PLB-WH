...
Ext.define("NJC.tr_picking_rackstock.FRMtr_picking_rackstock", {
  extend: "Ext.window.Window",
  alias: "widget.FRMtr_picking_rackstock",
  reference: "FRMtr_picking_rackstock",
  ...
  items: [
    ...
    { xtype: "tbspacer", height: 10 },
    {
      ...
      items: [
        {
          xtype: "grid",
          pid: "GRIDFRMtr_picking_rackstock",
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "tr_picking_rackstock/tr_picking_rackstocks",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMtr_picking_rackstock]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.tr_picking_rackstock.FRMtr_picking_rackstock_list_aju",
        {}
      );
      var GRIDtr_picking_rackstock = FRMmain.query(
        "grid[pid=GRIDFRMtr_picking_rackstock]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDtr_picking_rackstock.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDtr_picking_rackstock.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDtr_picking_rackstock = MODULEmain.query(
        "grid[pid=GRIDtr_picking_rackstock]"
      )[0];
      var GRIDFRMtr_picking_rackstock = PAGEthis.query(
        "grid[pid=GRIDFRMtr_picking_rackstock]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMtr_picking_rackstock.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "tr_picking_rackstock/tr_picking_rackstock",
              params,
              "POST",
