Ext.define("NJC.tr_free_rackstock.Ctr_free_rackstock", {
  extend: "Ext.app.ViewController",
  alias: "controller.Ctr_free_rackstock",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Ctr_free_rackstock");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDtr_free_rackstock]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("tr_free_rackstock");
      var popup = Ext.create("NJC.tr_free_rackstock.FRMtr_free_rackstock", {});
  ...
  GRIDtr_free_rackstock_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("tr_free_rackstock");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("tr_free_rackstock");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.tr_free_rackstock.FRMtr_free_rackstock", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMtr_free_rackstock = popup.query("grid[pid=GRIDFRMtr_free_rackstock]")[0];
      GRIDFRMtr_free_rackstock.getStore().on(
        "beforeload",
      ...
      GRIDFRMtr_free_rackstock.getStore().load();
