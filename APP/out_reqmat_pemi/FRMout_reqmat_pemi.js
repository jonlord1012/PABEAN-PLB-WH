...
Ext.define("NJC.out_reqmat_pemi.FRMout_reqmat_pemi", {
  extend: "Ext.window.Window",
  alias: "widget.FRMout_reqmat_pemi",
  reference: "FRMout_reqmat_pemi",
      ...
      items: [
        {
          ...
          store: {
            ...
            proxy: {
              ...
              url:
                vconfig.service_api +
                "out_reqmat_pemi/out_reqmat_pemis",
              reader: {
                type: "json",
                rootProperty: "Rows",
            ...
            { xtype: "tbspacer", width: 5 },
            {
              ...
              handler: function (cmp) {
                try {
                  ...
                  var GRID = FRM.query(
                    "grid[pid=GRIDFRMout_reqmat_pemi]"
                  )[0];
                  GRID.getStore()
                    .getDataSource()
                    .each(function (rec) {
  ...
  btsearch_aju_click: function (cmp) {
    try {
      ...
      var popup = Ext.create(
        "NJC.out_reqmat_pemi.FRMout_reqmat_pemi_list_aju",
        {}
      );
      var GRIDout_reqmat_pemi = FRMmain.query(
        "grid[pid=GRIDFRMout_reqmat_pemi]"
      )[0];
      ...
      GRIDwh_receivingpart_list_aju.on(
        ...
        function (xgrid, rec) {
          try {
            ...
            GRIDout_reqmat_pemi.getStore().load();
          } catch (ex) {
            COMP.TipToast.toast("Error", ex.message, {
              cls: "danger",
              delay: 2000,
      ...
      GRIDout_reqmat_pemi.getStore().on(
        "beforeload",
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDout_reqmat_pemi = MODULEmain.query(
        "grid[pid=GRIDout_reqmat_pemi]"
      )[0];
      var GRIDFRMout_reqmat_pemi = PAGEthis.query(
        "grid[pid=GRIDFRMout_reqmat_pemi]"
      )[0];
      ...
      var nvdetail = [];
      GRIDFRMout_reqmat_pemi.getStore().each(function (record) {
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(
              vconfig.service_api +
              "out_reqmat_pemi/out_reqmat_pemi",
              params,
              "POST",
