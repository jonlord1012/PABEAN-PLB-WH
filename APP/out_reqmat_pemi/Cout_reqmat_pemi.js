Ext.define("NJC.out_reqmat_pemi.Cout_reqmat_pemi", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cout_reqmat_pemi",
  init: function (view) {
    this.control({
      //
    });
  ...
  renderpage: function () {
    try {
      console.log("load Controller Cout_reqmat_pemi");
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDout_reqmat_pemi]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  ...
  btinput_receiving_partclick: function (cmp) {
    try {
      var MODULEmain = cmp.up("out_reqmat_pemi");
      var popup = Ext.create("NJC.out_reqmat_pemi.FRMout_reqmat_pemi", {});
  ...
  GRIDout_reqmat_pemi_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("out_reqmat_pemi");
  ...
  btdetail_rows_click: function (xgrid, rowIndex, colIndex, e, a, rec) {
    try {
      var MODULEmain = xgrid.up("out_reqmat_pemi");
      var me = this;
      xgrid.getSelectionModel().select(rowIndex);
      var popup = Ext.create("NJC.out_reqmat_pemi.FRMout_reqmat_pemi", {});
      popup.MODULEmain = MODULEmain;
      var FRM = popup.query("form")[0];
      var GRIDFRMout_reqmat_pemi = popup.query("grid[pid=GRIDFRMout_reqmat_pemi]")[0];
      GRIDFRMout_reqmat_pemi.getStore().on(
        "beforeload",
      ...
      GRIDFRMout_reqmat_pemi.getStore().load();
