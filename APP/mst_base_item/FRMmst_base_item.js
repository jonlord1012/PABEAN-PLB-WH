...
Ext.define("NJC.mst_base_item.FRMmst_base_item", {
  extend: "Ext.window.Window",
  alias: "widget.FRMmst_base_item",
  reference: "FRMmst_base_item",
  title: "Base Part Item/Material",
  modal: true,
  closeAction: "destroy",
  centered: true,
  ...
  handler_btsave_data: function (btn) {
    try {
      ...
      var GRIDmst_base_item = MODULEmain.query("grid[pid=GRIDmst_base_item]")[0];

      var FRM = PAGEthis.query("form")[0];
      var dtval = FRM.getValues(false, false, false, true);
      if (Ext.isEmpty(dtval.defcode)) {
        COMP.TipToast.msgbox("Error", "Kode Base harus diisi!", { cls: "danger", delay: 3000 });
        return false;
      }
      if (Ext.isEmpty(dtval.defname)) {
        COMP.TipToast.msgbox("Error", "Nama Base harus diisi!", { cls: "danger", delay: 3000 });
        return false;
      }
      Ext.MessageBox.confirm(
        ...
        function (button) {
          if (button === "yes") {
            ...
            var hasil = COMP.run.getservice(vconfig.service_api + "mst_base_item/mst_base_item", params, "POST", localStorage.getItem("NJC_JWT"));
            hasil.then(function (content) {
              ...
              if (val.success === "true") {
                ...
                GRIDmst_base_item.getStore().load();
                PAGEthis.close();
              } else {
