Ext.define("NJC.mst_base_item.Cmst_base_item", {
  extend: "Ext.app.ViewController",
  alias: "controller.Cmst_base_item",
  init: function (view) {
    this.control({
      //
    });
  ...
  btrefresh_click: function () {
    try {
      ...
      var GRID = Mainmodule.query("grid[pid=GRIDmst_base_item]")[0];
      GRID.getStore().load();
    } catch (ex) {
      COMP.TipToast.msgbox("Error", ex.message, { cls: "danger", delay: 2000 });
    }
  },
  btinput_mst_base_item_click: function (cmp) {
    try {
      var MODULEmain = cmp.up("mst_base_item");
      var popup = Ext.create("NJC.mst_base_item.FRMmst_base_item", {});
  ...
  GRIDmst_base_item_click: function (grid, rowIndex, colIndex, item, e, rec) {
    try {
      var MODULEmain = grid.up("mst_base_item");
